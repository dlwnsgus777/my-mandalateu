...
- [x] 주간 체크 현황 카드 구현
  - [x] 요일별 (Mo~Su) 완료 여부 시각화
  - [x] 이번 주 완료 TODO 개수 표시
  - [x] 지난 주 대비 증감률 표시
- [x] Streak(연속 달성) 카드 구현
  - [x] `completedAt` 기반 연속 날짜 계산 로직
  - [x] 🔥 N일 현재 기록 표시
  - [x] 최고 기록 별도 표시
- [x] 다가오는 마감일 카드 구현
  - [x] `deadline` 있는 셀 가까운 순 정렬
  - [x] 오늘(🔴) / 내일(🟡) / 3일 이내(🟢) 색상 구분
  - [x] 탭 시 해당 `BlockDetailScreen`으로 이동
- [x] 날짜 범위 토글 탭 추가 (주간/월간/전체)
  - [x] 선택 범위에 따라 통계 필터링 로직
