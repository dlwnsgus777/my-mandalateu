...
> 현재 상태: Phase 1 완료, Phase 2 ~ Phase 7 미착수
...
| Phase 2 | 도메인 엔티티 & Repository | ⬜ 미착수 |
| Phase 3 | 인증 (JWT + Security) | ⬜ 미착수 |
| Phase 4 | 만다라트 API | ⬜ 미착수 |
| Phase 5 | Strategy / ActionItem API | ⬜ 미착수 |
| Phase 6 | 대시보드 API | ⬜ 미착수 |
| Phase 7 | 예외 처리 & 검증 | ⬜ 미착수 |
...
- [ ] `actionitem/domain/Priority.kt` — enum class

### Repository

- [ ] `user/repository/UserRepository.kt` — `findByEmail` 메서드 포함
- [ ] `mandalart/repository/MandalartRepository.kt` — `findAllByUserId` 메서드 포함
- [ ] `strategy/repository/StrategyRepository.kt` — `findAllByMandalartId` 메서드 포함
- [ ] `actionitem/repository/ActionItemRepository.kt`
  - `findAllByStrategyId`
...
- [ ] `common/security/UserDetailsServiceImpl.kt`
  - `UserRepository.findByEmail` 기반 구현

### Auth 기능

- [ ] `auth/dto/SignupRequest.kt` — email, password, nickname (`@Valid` 포함)
- [ ] `auth/dto/LoginRequest.kt` — email, password
- [ ] `auth/dto/TokenResponse.kt` — accessToken, refreshToken, tokenType
- [ ] `auth/dto/SignupResponse.kt` — id, email, nickname
- [ ] `auth/service/AuthService.kt`
  - `signup`: 이메일 중복 확인, BCrypt 해시 후 저장
  - `login`: 비밀번호 검증, Access/Refresh 토큰 발급
  - `refresh`: Refresh 토큰 검증 후 Access 토큰 재발급
...
- [ ] `mandalart/dto/MandalartCreateRequest.kt` — title, coreGoal (`@NotBlank`)
- [ ] `mandalart/dto/MandalartUpdateRequest.kt` — title (nullable)
- [ ] `mandalart/dto/MandalartResponse.kt` — id, title, createdAt, updatedAt, strategies (중첩)
- [ ] `mandalart/dto/MandalartSummaryResponse.kt` — 목록 조회용 (strategies 미포함)

### Service & Controller

- [ ] `mandalart/service/MandalartService.kt`
...
- [ ] `mandalart/controller/MandalartController.kt`
...
- [ ] `strategy/dto/StrategyUpdateRequest.kt` — title, color, notes (모두 nullable)
- [ ] `strategy/dto/StrategyResponse.kt`
- [ ] `strategy/service/StrategyService.kt`
  - `update(userId, mandalartId, strategyId, request)`: 소유자 검증 포함
- [ ] `strategy/controller/StrategyController.kt`
  - `PATCH /api/v1/mandalarts/{mandalartId}/strategies/{strategyId}`

### ActionItem

- [ ] `actionitem/dto/ActionItemUpdateRequest.kt` — title, completed, deadline, priority, description (모두 nullable)
- [ ] `actionitem/dto/ActionItemResponse.kt`
- [ ] `actionitem/service/ActionItemService.kt`
  - `update(userId, mandalartId, strategyId, actionItemId, request)`
  - `completed = true`로 변경 시 `completedAt` 자동 설정, `false`로 변경 시 null 처리
- [ ] `actionitem/controller/ActionItemController.kt`
...
- [ ] `dashboard/dto/SummaryResponse.kt` — totalCount, completedCount, completionRate, strategyStats 목록
- [ ] `dashboard/dto/WeeklyStatsResponse.kt` — weekRange, dailyStats, thisWeekTotal, lastWeekTotal, changeRate
- [ ] `dashboard/dto/StreakResponse.kt` — currentStreak, bestStreak, lastCompletedDate
- [ ] `dashboard/dto/DeadlineResponse.kt` — actionItemId, title, strategyTitle, deadline, status (TODAY/TOMORROW/SOON)

### Service & Controller

- [ ] `dashboard/service/DashboardService.kt`
...
- [ ] `common/exception/ErrorResponse.kt` — code, message, timestamp
- [ ] `common/exception/GlobalExceptionHandler.kt` (`@RestControllerAdvice`)
  - `EntityNotFoundException` → 404
  - `AccessDeniedException` → 403
  - `MethodArgumentNotValidException` → 400 (필드별 오류 목록 포함)
  - `DuplicateEmailException` → 409
  - 기타 `Exception` → 500
- [ ] 각 Request DTO에 `@Valid` 어노테이션 점검
